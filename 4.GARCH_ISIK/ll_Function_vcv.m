function [sum_ll,sig_ll] = ll_Function_vcv(para, data)
...
%              ht, zt, s, k, eta, vartheta
    ...
    ll_IS = -0.5*(log(2*pi)+log(std_eta.^2)+log(eta.^2./std_eta.^2));
    ll_IK = -0.5*(log(2*pi)+log(std_vartheta.^2)+log(vartheta.^2./std_vartheta.^2));
    sum_ll = sum(LL_GCE+ll_IS+ll_IK);
    sig_ll = LL_GCE+ll_IS+ll_IK;
